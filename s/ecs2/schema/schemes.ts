...
export const u16 = asScheme<number>({
	size: 2,
	write: (b, x) => dataView(b).setUint16(0, x, endian),
	read: b => dataView(b).getUint16(0, endian),
})

export const u32 = asScheme<number>({
	size: 4,
	write: (b, x) => dataView(b).setUint32(0, x, endian),
	read: b => dataView(b).getUint32(0, endian),
})

export const f32 = asScheme<number>({
	size: 4,
	write: (b, x) => dataView(b).setFloat32(0, x, endian),
	read: b => dataView(b).getFloat32(0, endian),
})
